...
export default function FAQSection({ initialData }: Props) {
  ...
  const faqs =
    config.faqs && config.faqs.length > 0 ? config.faqs : DEFAULT_FAQS;
              ...
              return (
                <div
                  ...
                  className={`border rounded-2xl overflow-hidden transition-colors ${
                    isOpen
                      ? "bg-card-gray border-terracotta/30"
                      : "bg-absolute-black border-white/5 hover:border-white/10"
                  }`}
                >
                  <button
                  ...
                  >
                    <span
                      className={`font-bold pr-8 ${
                        isOpen ? "text-terracotta" : "text-light-gray"
                      }`}
                    >
                      {language === "hi" && faq.q_hi ? faq.q_hi : faq.q_en}
                    </span>
                    <span
                      className={`shrink-0 transition-transform duration-300 ${
                        isOpen ? "text-terracotta rotate-180" : "text-muted-ash"
                      }`}
                    >
                  ...
                  >
                    <p className="text-muted-ash text-sm leading-relaxed border-l-2 border-terracotta/30 pl-4">
                      {language === "hi" && faq.a_hi ? faq.a_hi : faq.a_en}
                    </p>
                  </div>
                </div>
              );
